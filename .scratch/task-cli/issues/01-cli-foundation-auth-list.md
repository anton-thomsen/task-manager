...
- [ ] A pnpm workspace package exposes a `task` bin (TypeScript), linkable via `pnpm link --global`
- [ ] `task auth <url> <token>` writes `~/.config/task/config.json` with mode 0600; `TASK_URL`/`TASK_TOKEN` env vars override the file
- [ ] `task list` calls the existing MCP endpoint (streamable HTTP, Bearer token) and renders tasks in aligned human-readable columns
- [ ] `task list --json` emits the structured data suitable for `jq`
- [ ] `task list` supports status, client, label, and assignee filters (matching the existing list tool's filters)
- [ ] Missing/invalid credentials produce a clear message pointing at `task auth`, exit code non-zero
- [ ] e2e coverage at the subprocess seam: tests invoke the real `task` binary with env vars pointing at the Playwright-managed e2e server, asserting on stdout and exit codes
- [ ] Visibility scoping is observable at the seam: a second user's token sees none of the first org's tasks
